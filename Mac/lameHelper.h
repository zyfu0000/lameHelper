...
#define LH_STARTED 0x0400 + 1
#define LH_COMPUTED 0x0400 + 2
#define LH_DONE 0x0400 + 3
#define LH_ERROR 0x0400 + 4

typedef void (*CALLBACK_PROC)(int msg, int percentage);
...
private:
	...
	int encode_x(char*, char*, settings_t, CALLBACK_PROC);
...
public:
	...
	int encode(char* pcm_in, char* mp3_out, settings_t settings, CALLBACK_PROC callback_proc);
	void* encode(char* pcm_in, char* mp3_out, settings_t settings, CALLBACK_PROC callback_proc, bool async);

	//Decode a mp3 to pcm
	int decode(char* mp3_in, char* pcm_out);
	int decode(char* mp3_in, char* pcm_out, CALLBACK_PROC callback_proc);
	void* decode(char* mp3_in, char* pcm_out, CALLBACK_PROC callback_proc, bool async);
};
