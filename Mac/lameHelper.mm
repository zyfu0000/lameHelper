...
int lameHelper::encode(char* pcm_in, char* mp3_out, settings_t settings, CALLBACK_PROC callback_proc)
{
	return encode_x(pcm_in, mp3_out, settings, callback_proc);
}

//The asynchronous version
void* lameHelper::encode(char* pcm_in, char* mp3_out, settings_t settings, CALLBACK_PROC callback_proc, bool async = false)
{
	if(async)
	{
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
            StaticParam_t* hg = new StaticParam_t;
...
int lameHelper::encode_s(void* param)
{
	...
	CALLBACK_PROC callback_proc = sp->callback_proc;
...
int lameHelper::encode_x(char* pcm_in, char* mp3_out, settings_t settings, CALLBACK_PROC callback_proc)
{
		...
		else	
		{
			...
			do
			{
				...
				if(callback_proc != NULL)
				{
					...
					callback_proc(LH_COMPUTED, percentage);
				}
			}while(read != 0);
...
void* lameHelper::decode(char* mp3_in, char* pcm_out, CALLBACK_PROC callback_proc, bool async)
{
	if(async)
	{
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
...
int lameHelper::decode(char* mp3_in, char* pcm_out, CALLBACK_PROC callback_proc)
{
	int read, i, samples;
